// Buyers view — buyers already on CareOwner: sortable, selectable table with
// dropdown filters; a row opens their full profile. Talent Acquisition
// Specialists (recruiters whose DVM networks hold buyers who aren't listed
// here) live on the Providers page — the cross-link below the table routes
// there for anyone who doesn't find the right buyer in this table.
...
const BuyersView = ({ tab, section, onSection, onEdit, onToast }) => {
  ...
  // Reach framing: buyers listed here plus the DVMs that buyer-referring
  // specialists say are exploring ownership — the demand you can't see in the
  // table below. Counts only firms open to Referring DVM Buyers.
  const hiddenDvms = TA_SPECIALISTS
    .filter(t => ((TA_PROVIDER_EXTRAS[t.id] || {}).openTo || {}).buyers)
    .reduce((n, t) => n + t.buyerReady, 0);

  return (
    <>
      <SubHeader crumbs={["Buyers"]} title="Buyers"
        subtitle="Browse buyers on CareOwner and invite the right ones to review your practice."
        actions={<>
      ...
      <div className="co-body">
        <div style={{ gridColumn: "2 / -1" }}>
          ...
          <button className="fb-cross" onClick={() => navigateTo("/providers")}>
            ...
            <span className="fb-cross__body">
              ...
              <span className="fb-cross__desc">
                Talent Acquisition Specialists on the Providers page know <b>{hiddenDvms} DVMs</b> exploring ownership
                who aren't listed here. Ask one for an introduction — CareOwner pays their referral fee, so you pay nothing extra.
              </span>
            </span>
          </button>
        </div>
